...
void FlightSim::DrawString(const char* str, uint8_t x, uint8_t y)
{
	PrintX = x;
	PrintY = y;
...
#define MAX_DIGITS 5
void FlightSim::DrawInt(int16_t val, uint8_t x, uint8_t y)
{
	PrintX = x;
	PrintY = y;
...
uint8_t FlightSim::DrawCurrency(int32_t val, uint8_t x, uint8_t y)
{
	uint8_t len = 0;
	PrintX = x;
	PrintY = y;
