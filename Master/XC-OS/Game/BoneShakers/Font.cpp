...
static unsigned char getFontLine(unsigned char data, int line_num) {
...
static void DrawCharOld(char c)
{
	uint8_t y = PrintY;
...
void BoneShaker::DrawString(const char* str, uint8_t x, uint8_t y)
{
	PrintX = x;
	PrintY = y;
...
#define MAX_DIGITS 5
void BoneShaker::DrawInt(int16_t val, uint8_t x, uint8_t y)
{
	PrintX = x;
	PrintY = y;
...
uint8_t BoneShaker::DrawCurrency(int32_t val, uint8_t x, uint8_t y)
{
	uint8_t len = 0;
	PrintX = x;
	PrintY = y;
