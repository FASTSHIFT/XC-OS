...
static const uint8_t TileImageData[] PROGMEM =
{
#include "TileData.h"
};
...
static uint8_t VisibleTileCache[VISIBLE_TILES_X * VISIBLE_TILES_Y];
static int8_t CachedScrollX, CachedScrollY;
static uint8_t AnimationFrame = 0;

// A map of which tiles should be on fire when a building is on fire
#define FIREMAP_SIZE 16
static const uint8_t FireMap[FIREMAP_SIZE] PROGMEM =
{ 1,2,3,1,2,3,1,3,1,2,3,2,2,1,3,1 };

static const uint8_t BuildingPopulaceMap[] PROGMEM =
...
void MicroCity::ResetVisibleTileCache()
{
...
void MicroCity::RefreshTile(uint8_t x, uint8_t y)
{
...
void MicroCity::SetTile(uint8_t x, uint8_t y, uint8_t tile)
{
	int screenX = x - CachedScrollX;
	int screenY = y - CachedScrollY;
...
void MicroCity::RefreshTileAndConnectedNeighbours(uint8_t x, uint8_t y)
{
	RefreshTile(x, y);
...
void MicroCity::RefreshBuildingTiles(Building* building)
{
	const BuildingInfo* info = GetBuildingInfo(building->type);
	uint8_t width = pgm_read_byte(&info->width);
	uint8_t height = pgm_read_byte(&info->height);
...
void MicroCity::DrawFilledRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t colour)
{
	for (int j = 0; j < h; j++)
	{
		for (int i = 0; i < w; i++)
...
void MicroCity::DrawRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t colour)
{
	for (int j = 0; j < w; j++)
	{
